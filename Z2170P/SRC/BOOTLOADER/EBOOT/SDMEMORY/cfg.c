...
BOOL BLWriteBootCfg( BOOT_CFG *pBootCfg )
{
    ...
    offset = IMAGE_XLDR_BOOTSEC_NAND_SIZE + IMAGE_EBOOT_BOOTSEC_NAND_SIZE; // 0x80000 + 0x40000
    ...
    pEBOOT = (UINT8*)IMAGE_WINCE_CODE_CA; // 0x80002000
    length = flashInfo.dwBytesPerBlock - flashInfo.wDataBytesPerSector; // 0x20000 - 0x800

    memset((VOID*)pEBOOT, 0xFF, flashInfo.dwBytesPerBlock);
    sector = block * flashInfo.wSectorsPerBlock; // 5 * 0x40 = 0x140
    offset = 0;
    while (offset < length) 
    {
	...
	{
        OALMSG(OAL_ERROR, (L"ERROR: EBOOT!BLWriteBootCfg: Flash block %d erase failed\r\n", block));
        goto cleanUp;
	}
...
//-----------------------------------------------------------------------------
BOOL BLReserveBootBlocks(BOOT_CFG *pBootCfg)
{
#ifndef BSP_NO_NAND_IN_SDBOOT
    BOOL rc = FALSE;
    HANDLE hFMD;
    ...
    UNREFERENCED_PARAMETER(pBootCfg);
	OALMSG(OAL_ERROR, (L"BLReserveBootBlocks: start!\r\n"));
    // Automatically mark the bootloader blocks as read-only/reserved
    regInfo.MemBase.Reg[0] = g_ulFlashBase; // The device is connected to CS0 mapped to 0x0800-0000
    hFMD = FMD_Init(NULL, &regInfo, NULL);
    if (hFMD == NULL)
	{
        OALMSG(OAL_ERROR, (L"ERROR: FMD_Init call failed!\r\n"));
    ...
    // Done    
    rc = TRUE;
	//OALMSG(OAL_ERROR, (L"BLReserveBootBlocks: end!\r\n"));
    //OALLog(L"\r\n");
